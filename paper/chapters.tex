...
The first chapter of this paper defines power diagrams, introduces some geometric notation and gives both geometric and algebraic interpretations of the distance metric together with some fundamental results about power diagrams.
Using a transformation from spheres to hyperplanes, the next chapter shows that there exists an affine equivalency between power diagrams in $d$ dimensions and polyhedra which can be described as the interchapter of halfspaces pointing upwards in $d+1$ dimensions.
...
In the last chapter of the paper, the implementation of both the algorithms and the incidence lattices accompanying this paper is described.

\chapter{Power Diagrams}
...
In this chapter, power diagrams will be defined in terms of the power of a point with respect to a sphere.
After introducing some geometric notation, some fundamental results about power diagrams will be shown which in the next chapter can be used to proof a relationship between power diagrams in $d$ dimensions and polyhedra in $d + 1$ dimensions.
...
\begin{figure}[tb]
    ...
    \begin{tikzpicture}
        ...
        \node[point] () at (p3) {};
    \end{tikzpicture}
    \caption[A simple two-dimensional power diagram]{The power diagram of the five sites $\PD(\left\{ s_1, \dots, s_5 \right\})$ is the set of their cells, whose boundaries are shown in the figure. Boundaries are defined by sets of points for which adjacent sites have equal minimizing power.}
    \label{fig:garage}
\end{figure}
...
\begin{definition}[Power Diagram]
    ...
    \begin{align}
        ...
        \end{aligned}\right..\\
        \intertext{The \define{power diagram $\PD(S)$} is the set of all cells corresponding to all sites and their non-empty interchapters and given by}
...
After introducing some geometric notation, the remainder of this chapter will show some properties of power diagrams in $d$ dimensions.
...
A \define{$j$-flat} $f$ is a set expressible as the interchapter of $d - j$, but no fewer, hyperplanes.
...
A set $P \subseteq R^d$ is called a \define{polyhedron} iff it is expressible as the interchapter of a finite number of halfspaces.
...
In this family, every face of a polyhedron in $C$ is itself a member of $C$ and every non-empty interchapter of any two members of $C$ is a face of each of them.
...
\section{Properties of Power Diagrams}
...
Since any of these differences can be expressed through the other two, their interchapter has higher dimensionality than a general interchapter of three hyperplanes can have.
\begin{lemma}[\lemcite{Observation 2}]
    \label{lem:cellsarepolyhedra}
    Let $s, t$ and $u$ be spheres in $\R^d$ with $d \geq 2$.
    If the spheres are not collinear, the interchapter of their chordales $\chor(s,t) \cap \chor(t, u) \cap \chor(u, s)$ is a $(d-2)$-flat.
...
Every cell in the power diagram is either unbounded or it is bounded by chordales or interchapters of other chordales.
\begin{lemma}[\lemcite{chapter 2.2, p.~80}]
...
\begin{proof}
    ...
    Since the cell of every sphere $s \in S$ is bounded by halfspaces defined by its $n - 1$ chordales (which are not necessarily all part of the power diagram), it can be expressed as the interchapter of $n - 1$ halfspaces and is therefore a $d$-polyhedron.
...
\begin{figure}[tb]
    ...
    \begin{tikzpicture}[x=0.5cm, y=0.5cm]
        ...
        \node[point] () at (p2) {};
    \end{tikzpicture}
    \caption[A two-dimensional power diagram with special cases]{The cell of site $s_2$ in this two-dimensional power diagram is empty. This can happen, if the sphere of a site is completely contained in a sphere of an other site. However, this condition is not sufficient. The sphere $s_4$ does have a non-empty cell, but its center is not part of its cell.}
...
While it is easy to calculate the chordale of two spheres, the main question to answer when constructing power diagrams is which chordales have non-empty interchapter with their cells and which $j$-faces (for $0 \leq j < d - 1$) shared by which cells exist.
...
\chapter{Embedding in \texorpdfstring{$d+1$}{d+1} Dimensions}
...
This will lead to a result identifying these power diagrams with polyhedra in $d+1$ dimensions which can be expressed as the interchapter of halfspaces pointing upwards.
...
\section{The Transformation Function}
...
\begin{figure}[tb]
    ...
    \begin{tikzpicture}[y=0.80pt, x=0.80pt, yscale=-1, xscale=1, inner sep=0pt, outer sep=0pt,font=\scriptsize]
        ...
        \end{scope}
    \end{tikzpicture}
    \caption[The projection function]{The projection function $\Pi$ maps the spheres $s_1$ and $s_2$ to hyperplanes in $d+1$ dimensions.
    The interchapter of the projections $\Pi(s_i)$ and the paraboloid $U$ are the orthogonal projections of $s_i$ onto the paraboloid.
    The orthogonal projection onto the $x_1$-$x_2$-plane of the interchapter of the two hyperplanes is the chordale of the two spheres.
...
\end{alignat}
The interchapter of $\Pi(s)$ and $U$ is therefore the vertical projection of $s$ onto $U$.
...
\begin{lemma}
    ...
    The transform $\Pi$ is a bijection from spheres in $\R^d$ to hyperplanes in $\Ha$ with nonempty interchapter with $U$.
\end{lemma}
\begin{proof}
    ...
    $\Pi$ is also surjective if every interchapter of $U$ with a hyperplane in $\Ha$ is an ellipsoid which maps to a sphere if projected to $h_0$.
    ...
    The interchapter of $h$ with $U$ is given by
    \begin{align}
    ...
    \begin{alignat}{2}
        ...
        &\Leftrightarrow\quad& x_{[d]}^Tx_{[d]} + n_{[d]}^Tx_{[d]} - b &= 0,
    \end{alignat}
    so the projection of the interchapter is indeed a sphere if $b > -\frac{1}{4}n_{[d]}^Tn_{[d]}$, a point if $b = -\frac{1}{4}n_{[d]}^Tn_{[d]}$ and $h$ does not intersect $U$ otherwise.
\end{proof}
...
This directly implies the following lemma, which connects chordales to the newly formed hyperplanes by recognizing that the interchapter of two hyperplanes in $\R^{d+1}$ contains projected points with the same power value for the respective spheres.
...
\section{Duality of points and hyperplanes}
\label{sub:duality_of_points_and_hyperplanes}
The hyperplanes established with the mapping $\Pi$ will now be connected to specific \define{polar points} to establish a bijective mapping between $j$-flats defined by the interchapter of these hyperplanes and $(d-j)$-flats in this dual domain.
...
\begin{proof}
    ...
    Then $\Delta(F)$ is not expressible as the interchapter of hyperplanes in $\Ha$ and therefore, there must exist two different $d^{(1)}, d^{(2)} \in \Delta(F)$ with $d_{[d]}^{(1)} - d_{[d]}^{(2)} = 0$.
...
\section{Power Diagrams and Polyhedra}
...
\begin{figure}[tb]
    ...
    \begin{tikzpicture}[y=0.80pt, x=0.80pt, yscale=-1.000000, xscale=1.000000, inner sep=0pt, outer sep=0pt]
        \begin{scope}[cm={{1.25,0.0,0.0,-1.25,(0.0,900.0)}}]
            ...
            % Center Nodes
            \tikzstyle{interchapter} = [sphere center, tumblue, minimum size=3pt];
            \node[interchapter] at (s1) {};
            \node[interchapter] at (p1) {};
            \node[interchapter] at (d1) {};
            \node[interchapter] at (s4) {};
            \node[interchapter] at (p4) {};
            \node[interchapter] at (d4) {};

            \node[interchapter] at (s2) {};
            \node[interchapter] at (p2) {};
            \node[interchapter] at (d2) {};

            \node[interchapter] at (s3) {};
            \node[interchapter] at (p3) {};
            \node[interchapter] at (d3) {};
        ...
        \end{scope}
    \end{tikzpicture}
    \caption[The polarization function]{The polar point $\Delta(\Pi(s_i))$ of the hyperplane $\Pi(s_i)$ shares the first $d$ coordinates with the sphere center $z_i$.
...
\begin{theorem}[\lemcite{Theorem 4}]
    ...
    For any $(d+1)$-polyhedron P which is expressible as the interchapter of upper halfspaces of hyperplanes with normal vectors with their $(d+1)$th coordinate equal to $-1$, there exists an affinely equivalent power diagram in $h_0$ and vice versa.
\end{theorem}
\begin{proof}
    Let $P = \bigcap_{i=1}^n h_i^\leq$ be the interchapter of upper halfspaces of hyperplanes $h_i$ with normal vectors with $(d+1)$th coordinates equal to $1$ such that for facet $f_i$ of $P$ it holds that $f_i \subseteq h_i$.
...
\end{proof}
To also describe polyhedra $P$ generated using the interchapter of upper halfspaces, one can inspect the reflection of $P$ through $h_0$ as described in \cite{aurenhammer1987power}.
...
\end{theorem}
\begin{proof}
    By \cref{thm:equivalentpolyhedron} there exists an affinely equivalent polyhedron $P$ for each $\PD(S)$ in $h_0$ which is defined by the interchapter of the upper halfspaces of the hyperplanes $H = \left\{ h_i \mid h_i = \Pi(s_i), 1 \leq i \leq n \right\}$ with $n = \abs{S}$ and vice versa.
...
\chapter{Constructing Power Diagrams}
\label{sec:constructing_power_diagrams}
In this chapter, the theoretical results of the previous chapters will be applied to the formulation of algorithms to construct the power diagram $\PD(S)$ of some set of spheres $S$ in $d$ dimensions.
...
This means that all chordales which exist in the power diagram and their interchapters which form flats of lower dimensionality are represented as faces in this polyhedron as can be seen from \cref{lem:verticalprojection}.
...
\begin{figure}[tb]
    ...
    \end{tikzpicture}
    \caption[Two dual polytopes sharing an incidence lattice]{The polytope on the left is given as the convex hull of $\left\{ v_1, \dots, v_4 \right\}$.
    ...
    \begin{algorithmic}[1]
        \Function{FindZeroFaces}{$S$}
            ...
            \For{$G$ \textbf{in} $\Ge$}\Comment{Find $0$-faces}
                \State $P \gets \bigcap_{s_1, s_2 \in G} \chor(s_1, s_2)$\Comment{Find interchapter of all pairs of spheres in $G$}.
        ...
        \EndFunction
    \end{algorithmic}
    \caption{Naive approach to finding $0$-faces of power diagrams}
    \label{alg:zerofaces}
\end{algorithm}
...
\begin{proof}
    ...
    Finding the interchapter $p$ in line~5 amounts to solving a system of linear equations with a matrix of size $(d+1) \times d$, which takes $\Oh(d^3)$ time.
...
Since this naive algorithm is not applicable in practice due to its running time, these questions were not explored further and it was only used as a test implementation for the efficient algorithm presented in the next section.
...
This chapter first describes the general architecture of the code written for this interdisciplinary project.
...
\begin{figure}[tb]
    ...
    \end{tikzpicture}
    \caption[Adding a face to an incidence lattice]{The left figure shows part of an incidence lattice in the process of being filled before the insertion of the new face $\left\{ 1,2,3 \right\}$. To identify the red dashed edges which need to be bisected by the new face, lower and upper boundaries have to be found.
...
Suppose the face $f_{123} = \left\{ 1, 2, 3 \right\}$ which is defined by the interchapter of the cells represented by the nodes $1$, $2$ and $3$ should be added to the lattice.
...
Since the implementation of the naive algorithm in \src{PowerDiagramNaive.cpp} follows \cref{alg:zerofaces} rather directly, the following section will only describe the implementation of the efficient algorithm.
...
\begin{algorithm}[tbp]
    ...
    \begin{algorithmic}[1]
        ...
        \EndProcedure
    \end{algorithmic}
    \caption{Finding $\IL_b$ and dualizing it}
...
\begin{algorithm}[tbp]
    ...
    \begin{algorithmic}[1]
        ...
        \EndFunction
    \end{algorithmic}
    \caption{Finding the direction of an extremal ray}
    \label{alg:ridges}
\end{algorithm}
...
After introducing some general properties of power diagrams, the main focus of the project lies on the affine equivalency of power diagrams in $d$ dimensions and those polyhedra in $d+1$ dimensions which can be described as the interchapter of half spaces which point upwards in $x_{d+1}$ direction.
...
The following two sections will describe a typical build process on both Linux and Windows, which can be customized via options described in the documentation of CMake \cite{cmake-tutorial}.
...
$ make clean
    \end{minted}
    \caption[Using \var{make} to build the program]{A simple invocation of \var{make} creates a debug build using bundles libraries.
...
\begin{figure}[p]
    ...
    \caption[Building dependencies using CMake]{The \src{third-party} directory contains a CMake script which downloads and builds the libraries needed by the \var{powerdiagram} executable.
...
\begin{figure}[p]
    ...
    \caption[Choosing a toolchain in CMake]{After clicking the \enquote{Configure}-button, CMake prompts the user to choose a generator or toolchain.
...
\begin{figure}[p]
    ...
    \caption[Choosing the build type in CMake]{To change the build type of the libraries from the default optimized release build to a debug build, add the according entry after the \enquote{Configure} step.}
    \label{fig:cmake_3}
\end{figure}
\begin{figure}[p]
    \centering
    \includegraphics[width=0.9\linewidth]{cmake_4.png}
    \caption[Building the program using CMake]{To build the program executable, CMake has to be run again with the top level directory as the source folder.}
    \label{fig:cmake_4}
\end{figure}
...
\begin{figure}[tb]
    ...
    \caption[A Visual-Studio solution created with CMake]{The Visual Studio solution created by CMake for the program can be used to build the code in both debug and release mode.
...
For a complete help, use options --help or --helpfull.
  ...
  Flags from (...)/powerdiagram_main.cpp:
    ...
    \end{minted}
    \caption[The output of running \var{./powerdiagram -help}]{The output of running \var{./powerdiagram -help}.}
    \label{lst:help}
\end{listing}

This chapter shows how to use the program to create a visualization of a two-dimensional power diagram in the \LaTeX-package tikz with \cref{fig:garage} as an example.
...
$ make TIKZ_OUT=1 garage
    \end{minted}
    \caption[Creating a tikz-picture of a power diagram]{To create a tikz picture from sphere centers and radii defined in text files, the \var{./powerdiagram -draw} parameter can be used.
...
ee p3 s1 s5 d0.447214 0.894427
    \end{minted}
    \caption[The intermediate output format for power diagrams]{The intermediate output format generated by \var{./powerdiagram -draw}.
