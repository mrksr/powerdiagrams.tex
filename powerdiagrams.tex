        ...
        \Function{PowerDiagram}{$S$}
            ...
            \For{face $f$ \textbf{in} $\IL_b$}\Comment{Dualize $\IL_b$}
                \If{$f$ \textbf{is a} facet}
                    \State Find hyperplane $h$ containing $f$
...
To improve readability, code presented in this paper will not be pseudo code rather than concrete \CC.
    ...
    \begin{algorithmic}[1]
        ...
        \Function{FindGroups}{$\IL, V_{\textnormal{minimals}}$}\Comment{Find maximal existing subfaces of $V_{\textnormal{minimals}}$}
            ...
            \For{$v \in V_{\textnormal{minimals}}$}
                \State $G \gets \Call{FindNodesBFS}{\IL, v_0, \pred{isGroup}, \pred{isGroup}, \pred{Successors}(\IL)}$
        ...
        \Function{FindLUBs}{$\IL, V_{\textnormal{minimals}}$}\Comment{Find minimal existing superfaces of $V_{\textnormal{minimals}}$}
            ...
            \State \Return \Call{FindNodesBFS}{$\IL, v_0, \pred{isUb}, \pred{isNotUb}, \pred{Successors}(\IL)$}
        \EndFunction
        \Statex
