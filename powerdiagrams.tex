...
\end{lemma}
\begin{proof}
    Let $F = f + X \subset \R^{d+1}$ be some flat with $f \in \R^{d+1}$, $\dim(X) = j$ and
    \begin{align}
    ...
    Then let \Wlog \todo{Was ist mir den senkrechten Ebenen?}$N = \left\{ n \in X^\bot \mid n_{d+1} = 1 \right\}$ the set of all possible normal vectors and
    \begin{align}
        \Delta(F) &= \Delta(h(N))\\
    ...
    Is is defined by the affine hull of the polar points of $h(n_i)$ for any suitable basis $\left\{ n_1, \dots, n_{d - j + 1} \right\}$ of $X^\bot$.
    It is surjective since any $(d - j$)-flat which is the affine hull of points $\left\{ f_1, \dots, f_{d - j + 1} \right\}$ one can find suitable $n_i$ to form a basis and $f$ to define a $j$-flat which is its preimage.
    ...
    \end{align}
    is a hyperplane, for which $\Delta(\Delta(F)) = F$, which proves that $\Delta$ is involutory.
...
Having established the bijectivity of $\Pr$ it can be shown that power diagrams in $\R^d$ can be identified with boundaries of $(d+1)$-polyhedra.
\begin{theorem}
    \label{thm:equivalentpolyhedron}
    For any $(d+1)$-polyhedron P which is expressible as the intersection of upper halfspaces, there exists an affinely equivalent power diagram in $h_0$ and vice versa.
\end{theorem}
\begin{proof}
...
\begin{proof}
    ...
    \Cref{lem:relativepositions} now implies that exactly then $p_i, p_j \in \Delta(x)$ and each other point $p \in M \setminus \left\{ p_i, p_j \right\}$ is above $\Delta(x)$.
