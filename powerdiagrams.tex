    ...
    \begin{algorithmic}[1]
        \Function{FindZeroFaces}{$S$}
            ...
            \For{$G$ \textbf{in} $\Ge$}\Comment{Find $0$-faces}
                \State \label{alg:0f:step31}$P \gets \bigcap_{s_1, s_2 \in G} \chor(s_1, s_2)$\Comment{Find intersection of all pairs of spheres in $G$}.
            ...
            \For{$0$-faces $p_1, p_2 \in \IL$}\Comment{Merge overdefined $0$-faces}
                \If{$p_1 = p_2$}\Comment{Merge $p_1$ and $p_2$}
                    ...
                    \State $\IL \gets \IL - p_2$\Comment{Remove $p_2$ from $\IL$}
...
\begin{algorithm}[tb]
    ...
    \begin{algorithmic}[1]
        ...
        \EndFunction
    \end{algorithmic}
    \caption{$0$-faces of Power Diagrams}
    \label{alg:embedding}
\end{algorithm}
...
The implementation was written in \CC, making heavy use of features introduced by the \CC11 standard which introduces a more functional and generic style into the language.
...
\label{sub:impl_incidence_lattice}
...
The implementation of these lattices in \src{IncidenceLattice.hpp} therefore uses the graph datastructure in \src{BidirectionalGraph.hpp} as an internal representation.
...
\begin{algorithm}[tbp]
    ...
    \begin{algorithmic}[1]
        \Function{FindNodesBFS}{$\IL, v_0, \pred{filter}, \pred{cont}, \pred{next}$}
            ...
            \While{$V_{\textnormal{tovisit}}$ \textbf{is not} empty}
                \State $(v, V_{\textnormal{tovisit}}) \gets V_{\textnormal{tovisit}}$ \Comment{Pop the first element of $V_{\textnormal{tovisit}}$}

                \If{$\pred{cont}(v)$}
                    \For{$n$ \textbf{in} $\pred{next}(\IL, v)$}
                        \If{$n$ \textbf{not in} $V_{\textnormal{visited}}$}
                            \State $V_{\textnormal{visited}} \gets V_{\textnormal{visited}} \cup \left\{ n \right\}$
                            \State $V_{\textnormal{tovisit}} \gets (V_{\textnormal{tovisit}}, n)$ \Comment{Append $n$ to $V_{\textnormal{tovisit}}$}
                        \EndIf
                    \EndFor
                \EndIf
        ...
        \EndFunction
        \Statex
        \Function{FindGroups}{$\IL, V_{\textnormal{minimals}}$}\Comment{Find maximal existing subfaces of $V_{\textnormal{minimals}}$}
        ...
        \EndFunction
        \Statex
        \Function{FindLUBs}{$\IL, V_{\textnormal{minimals}}$}\Comment{Find minimal existing superfaces of $V_{\textnormal{minimals}}$}
...
During this process, the lattice is usually \define{shallow} in the sense that it contains minimal nodes (the vertices) and some maximal nodes (the $d$-dimensional facets), but it does not contain $j$-faces for $1 \leq j < d$.
...
Since the lattice should not contain transitive edges, it is necessary to identify the maximal subfaces (which in the code are called \define{groups}) and minimal superfaces (which in the code are called \define{lubs}) which are exactly the edges whose edges have to be modified.
