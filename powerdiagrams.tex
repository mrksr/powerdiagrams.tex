...
Using this definition, a pair $s = (p, \sqrt{w(p)})$ of a point and its weight is called a \define{site} of the power diagram, which can be interpreted as a sphere with a radius of $\sqrt{w(p)}$.
...
\subsection{The Transform to \texorpdfstring{$d+1$}{d+1} Dimensions}
...
Let $U$ denote the paraboloid $x_{d+1} = x^Tx$ with $x = \left( x_1, \dots, x_d \right)^T$.
\begin{definition}[Transformation Function]
    Let $s = (z, r^2)$ be a $d$-dimensional sphere with $z \in h_0$.
    The \define{transformation function $\Pi$} maps this sphere to the hyperplane
    \begin{align}
        \Pi(s) &\coloneqq \left\{ \left( x, x_{d+1} \right) \in \R^{d+1} \mid x_{d+1} = 2x^Tz - z^Tz + r^2 \right\}\\
    &= \left\{ \left( x, x_{d+1} \right) \in \R^{d+1} \mid \left( 2 \cdot z, -1 \right)^T\left( x, x_{d+1} \right) = z^Tz - r^2 \right\}.
    \end{align}
\end{definition}

This transform has a direct connection to $U$ since
\begin{align}
    \Pi(s) \cap U &= \left\{ (x, x_{d+1}) \in \R^{d+1} \mid x_{d+1} = 2x^Tz - z^Tz + r^2 = x^Tx \right\}
\end{align}
where it holds that
\begin{alignat}{2}
    &&2x^Tz - z^Tz + r^2 &= x^Tx\\
    &\Leftrightarrow\quad& \dist(x, z) &= r^2\\
    &\Leftrightarrow\quad& x &\in s.
\end{alignat}
...
\begin{proof}
    ...
    Let $(n, 1) \in \R^{d+1}$ be a normal vector with $n \neq 0$, let $b \in \R$ be an offset and $h = \left\{ y \in \R^{d+1} \mid (n, 1)^T y = b \right\}$ a hyperplane in $\R^{d+1}$ which is not parallel to the $x_{d+1}$ axis.
    The projection to $h_0$ of the intersection of $h$ with $U$ is given by
    \begin{align}
        h \cap U = \left\{ (x, x_{d+1}) \in \R^{d+1} \mid x_{d+1} = b - n^T x = x^T x \right\}
    \end{align}
    where
    \begin{alignat}{2}
        &&b - n^Tx &= x^Tx\\
        &\Leftrightarrow\quad& x^Tx + n^Tx - b &= 0.
    \end{alignat}
    ...
    \begin{alignat}{2}
        &&(x - v)^2 &= \alpha.
        \intertext{Now let $\alpha = b + \frac{1}{4} n^Tn$ and $v = \frac{1}{2}n$. Then}
        &&(x - \frac{1}{2}n)^2 &= b + \frac{1}{4} n^Tn\\
        &\Leftrightarrow\quad&x^Tx - n^Tx + \frac{1}{4} n^Tn &= b + \frac{1}{4} n^Tn\\
        &\Leftrightarrow\quad& x^Tx + n^Tx - b &= 0
    \end{alignat}
    so the projection of the intersection is indeed a sphere if $b > -\frac{1}{4}n^Tn$, a point if $b = -\frac{1}{4}n^Tn$ and $h$ does not intersect $U$ otherwise.
\end{proof}
...
\begin{observation}
    ...
    \begin{align}
        \pow(x, s) = \dist(x, x^\prime) - \dist(x, x^{\prime\prime})
    \end{align}
\end{observation}
\begin{proof}
    Since the projections of $x$ did not change the first $d$ coordinates, it holds that $\dist(x, x^\prime) = x^Tx$ and $\dist(x, x^{\prime\prime}) = 2x^Tz - z^Tz + r^2$, with their difference being $(x - z)^T(x - z) - r^2 = \pow(x, s)$.
\end{proof}
...
\subsection{Duality of points and hyperplanes in $\R^{d+1}$}
\label{sub:duality_of_points_and_hyperplanes}
The hyperplanes established with the mapping $\Pi$ will now be connected to specific \define{polar points} to establish a bijective mapping between $j$-faces defined by the intersection of hyperplanes and $(d-j)$-faces in this dual domain.

\begin{definition}[Polarity Function]
    Let $n \in \R^{d}$ and $n_{d+1} \in \R$.
    The \define{polarity function $\Delta$} maps the hyperplane $h = \left\{ (x, x_{d+1}) \in \R^{d+1} \mid x_{d+1} = n^Tx + n_{d+1} \right\}$ to the point
    \begin{align}
        \Delta(h) &\coloneqq \left( \frac{1}{2} n, -n_{d+1} \right) \in \R^{d+1}
        \intertext{and any face $f \subseteq \R^{d+1}$ to}
        \Delta(f) &\coloneqq \bigcup_{f \subseteq h} \Delta(h).
    \end{align}
\end{definition}
...
The following lemma shows that the polarity function maps polyhedra to their duals.
\begin{lemma}
    $\Delta$ is an involutory function and a bijection between $j$-faces and $(d-j)$-faces in $\R^{d+1}$ (for $0 \leq j \leq d$).
\end{lemma}
\begin{proof}
    \todo[inline]{Weil isso.}
\end{proof}
...
\end{lemma}
\begin{proof}
    Let $(x, x_{d+1}),(n, n_{d+1}) \in \R^{d+1}$ and $h = \left\{ (y, y_{d+1}) \in \R^{d+1} \mid y_{d+1} = n^Ty + n_{d+1} \right\}$ a hyperplane in $\R^{d+1}$.
    Then by definition
    \begin{align}
        \Delta(h) &= \left( \frac{1}{2}n, -n_{d+1} \right)\\
        \Delta(p) &= \left\{ (y, y_{d+1}) \in \R^{d+1} \mid y_{d+1} = 2x^Ty - x_{d+1} \right\}\\
    \end{align}
    Any point $(y, y_{d+1}) \in \R^{d+1}$ is above $\Delta{p}$ if
    \begin{align}
        2x^Ty - y_{d+1} - x_{d+1} > 0
    \end{align}
    and on or below the hyperplane when it is equal or smaller.
    Inserting $\Delta(h)$ into the equation yields
    \begin{align}
        2x^T \left( \frac{1}{2}n \right) + n_{d+1} - x_{d+1} = n^Tx - x_{d+1} + n_{d+1}
    \end{align}
    which is equal to the expression determining the relative position of $(x, x_{d+1})$ to $h$.
\end{proof}
