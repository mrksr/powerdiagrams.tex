...
$\PD(S)$ therefore is a cell complex in $\R^d$.
    ...
    \begin{align}
        ...
        \Delta &: \left\{ \begin{aligned}
            ...
            f &\mapsto \begin{dcases}
                \begin{pmatrix} \frac{1}{2} a \\ -a_{d+1} \end{pmatrix} &\qquad \textnormal{if } h \in \Ha \textnormal{ with } x \in h \Leftrightarrow x_{d+1} = a^Tx_{[d]} + a_{d+1}\\
                \bigcup_{\substack{f \subseteq h,\\h \in \Ha}} h &\qquad \textnormal{if } \dim(f) < d
            \end{dcases}
        \end{aligned}
        \right..
    \end{align}
...
\begin{proof}
    ...
    Suppose that $F = p + \left\{ 0 \right\}$ is a $0$-flat.
    Then $\left\{ 0 \right\}^\bot = \R^{d+1}$ and \Wlog $\Delta(F)$ can be computed as
    \begin{align}
    ...
    \end{align}
    This is a hyperplane for which $\Delta(\Delta(p)) = p$, which proves that $\Delta$ is involutory.
...
\begin{figure}[tb]
    ...
    \begin{tikzpicture}[font=\scriptsize]
        ...
        (d1234) node[lattice, tumgreen, label={above:$\left\{ 1, 2, 3, 4 \right\}$}] {};
        ...
        (v1) node[corner, tumgreen] {};
    \end{tikzpicture}
    \caption{This figure shows a polytope on the left defined by the convex hull of the vertices $\left\{ v_1, \dots, v_4 \right\}$ which has four $0$-faces (the $v_i$), four $1$-faces connecting the vertices and one $2$-face, the complete polytope, which is incident to all the $1$-faces.
    This combinatorical structure is reflected in its incidence lattice which can be seen in the center.
    It is a DAG with the $v_i$ as the minimal nodes and the $2$-faces as maximal nodes.
    The same incidence lattice also describes the faces shared by the spheres $\left\{ s_1, \dots, s_4 \right\}$ in the Voronoi diagram on the right.}
